...
def phase2_local_plan_n(
    ...
    task: str, use_offer: bool = True, verbose: str = "full",
) -> Dict[str, LocalPlan]:
    """images: Phase 1과 동일한 agent별 이미지 리스트.

    원본 2-agent observe_and_draft() + coordinate()를 그대로 N-agent로 일반화한
    2라운드 구조:
      Round A (DRAFT)      : 각 agent가 offer 요약만 보고 초안 생성 (N번 VLM 호출)
      Round B (COORDINATE) : 각 agent가 자기 draft + 다른 N-1명의 '전체 draft plan'을
                              모두 broadcast 받아서 최종 plan으로 다듬음 (N번 VLM 호출)
    N=2일 때는 원본과 100% 동일한 구조(자기 draft + 상대 1명의 draft)이고,
    N=4로 늘어나면 다른 3명의 draft를 전부 보여주는 것뿐 — 로직은 바뀌지 않는다.
    ...
    with _TPE(max_workers=n) as ex:
        ...
        draft_results = [f.result() for f in futs]

    drafts: Dict[str, LocalPlan] = {}
    for idx, aid in enumerate(agent_ids):
        raw, logp = draft_results[idx]
        if verbose == "full":
            _log(f"{aid} DRAFT RAW", raw)
    ...
    with _TPE(max_workers=n) as ex:
        futs = [ex.submit(_p2._vlm_with_retry, img, p, True) for img, p in zip(images, coord_prompts)]
        coord_results = [f.result() for f in futs]

    plans: Dict[str, LocalPlan] = {}
    for idx, aid in enumerate(agent_ids):
        raw, logp = coord_results[idx]
        if verbose == "full":
