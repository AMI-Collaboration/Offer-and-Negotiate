...
def run_n(
    ...
    # ── N=2 / N=4(혹은 그 외) 체크 ────────────────────────────────────────
    if n_agents == 2:
        print(f"  [MODE] N=2 — 원본 2-agent 파이프라인과 동일한 구조로 실행")
    elif n_agents == 4:
        print(f"  [MODE] N=4 — 확장된 다중 agent 파이프라인으로 실행 (동일 알고리즘)")
    else:
        print(f"  [MODE] N={n_agents} — 임의 N-agent 파이프라인으로 실행 (동일 알고리즘)")
    ...
    plans = phase2_local_plan_n(offers, images, agent_ids, task, use_offer=use_offer, verbose=verbose)
    ...
    return {
        ...
        "n_agents": n_agents,
        "agent_ids": agent_ids,
        "flags": {"use_offer": use_offer, "use_negotiation": use_negotiation, "use_human_query": use_human_query},
